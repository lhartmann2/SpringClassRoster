...
public class CourseDaoDB implements CourseDao{
    ...
    public List<Course> getAllCourses() {
        ...
        associateTeachersAndStudents(courses);
        return courses;
    }
    ...
    private void associateTeachersAndStudents(List<Course> courses) {
        for(Course c : courses) {
            c.setTeacher(getTeacherForCourse(c.getId()));
            c.setStudents(getStudentsForCourse(c.getId()));
        }
    }

    private void insertCourseStudent(Course course) {
        final String INSERT_COURSE_STUDENT = "INSERT INTO course_student(courseId, studentId) "+
                "VALUES(?,?)";
